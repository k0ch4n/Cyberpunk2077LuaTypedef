---@meta

---@class gamedataChoiceCaptionPart_Record: gamedataTweakDBRecord
gamedataChoiceCaptionPart_Record = {}

---@param fields? gamedataChoiceCaptionPart_Record
---@return gamedataChoiceCaptionPart_Record
function gamedataChoiceCaptionPart_Record.new(fields) end

---@return gamedataChoiceCaptionPartType_Record
function gamedataChoiceCaptionPart_Record:PartType() end

---@return gamedataChoiceCaptionPartType_Record
function gamedataChoiceCaptionPart_Record:PartTypeHandle() end
