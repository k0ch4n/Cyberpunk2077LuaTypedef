---@meta

---@class gamedataMappinUISpawnProfile_Record: gamedataTweakDBRecord
gamedataMappinUISpawnProfile_Record = {}

---@param fields? gamedataMappinUISpawnProfile_Record
---@return gamedataMappinUISpawnProfile_Record
function gamedataMappinUISpawnProfile_Record.new(fields) end

---@return Float
function gamedataMappinUISpawnProfile_Record:DespawnDistance() end

---@return Float
function gamedataMappinUISpawnProfile_Record:SpawnDistance() end
