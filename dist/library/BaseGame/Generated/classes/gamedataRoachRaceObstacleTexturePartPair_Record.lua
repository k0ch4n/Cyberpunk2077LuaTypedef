---@meta


---@class gamedataRoachRaceObstacleTexturePartPair_Record: gamedataTweakDBRecord
gamedataRoachRaceObstacleTexturePartPair_Record = {}


---@param fields? gamedataRoachRaceObstacleTexturePartPair_Record
---@return gamedataRoachRaceObstacleTexturePartPair_Record
function gamedataRoachRaceObstacleTexturePartPair_Record.new(fields) end

---@return CName
function gamedataRoachRaceObstacleTexturePartPair_Record:ImageTexturePart() end

---@return CName
function gamedataRoachRaceObstacleTexturePartPair_Record:Name() end
